...
const UMapDisplay = ({ prompt, isLoading = false, visibleLayers = [], generatedMap, layers = [], isSharedView = false, isFullscreen = false }: UMapDisplayProps) => {
  ...
  const initializeMap = () => {
    ...
    const planIgnLayer = L.tileLayer('https://wxs.ign.fr/essentiels/geoportail/wmts?REQUEST=GetTile&SERVICE=WMTS&VERSION=1.0.0&STYLE=normal&TILEMATRIXSET=PM&FORMAT=image/png&LAYER=GEOGRAPHICALGRIDSYSTEMS.PLANIGNV2&TILEMATRIX={z}&TILEROW={y}&TILECOL={x}', {
      attribution: '© IGN',
      maxZoom: 18
    });
    ...
    <Card className="shadow-card">
      ...
      <CardContent>
        <div className="relative bg-muted/20 rounded-lg overflow-hidden">
          ...
          {/* Drawing Tools Overlay - Bottom Left */}
          <div className="absolute bottom-2 left-2 z-[400] bg-white/90 backdrop-blur-sm rounded px-2 py-1 shadow-sm border">
          ...
          <div className="absolute top-4 left-1/2 transform -translate-x-1/2 z-[400] bg-white/95 text-gray-900 px-4 py-2 rounded-lg backdrop-blur-sm shadow-lg border">
            {isEditingTitle ? (
              <div className="flex items-center gap-2">
                <input
                  type="text"
